...
contains
  ...
  pure recursive subroutine search_cp(xsq, start, cp, best_ll, best_cp)
    ...
    if (pos == 0) then
      call evaluate_segmentation(xsq, cp, best_ll, best_cp)
      return
    end if
    ! The upper bound for a valid candidate index is n - (k - pos)
    do i = start, n - (k - pos)
      cp(pos) = i
      call search_cp(xsq, i+1, cp, best_ll, best_cp)
      cp(pos) = 0  ! reset for the next candidate
    end do
  end subroutine search_cp

  pure subroutine evaluate_segmentation(xsq, cp, best_ll, best_cp)
    ...
    do i = 1, k
       ...
       sumsq = sum(xsq(seg_start:seg_end))
